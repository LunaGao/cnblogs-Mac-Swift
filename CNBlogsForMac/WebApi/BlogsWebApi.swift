...
class BlogsWebApi : NSObject {
    
    func blogsListRequest(pageIndex:Int , pageSize:Int, callback:(NSArray) -> Void){
        ...
        baseApi.basicRequest { (access_token) -> Void in
            ...
            let urlstr = "https://api.cnblogs.com/api/blogposts/@sitehome?pageIndex=\(pageIndex)&pageSize=\(pageSize)"
...
//                    NSLog("%@", String.init(data: data!, encoding: NSUTF8StringEncoding)!)
    ...
    func blogBodyRequest(blogId:Int , callback:(NSData) -> Void){
        let baseApi = BaseWebApi()
        baseApi.basicRequest { (access_token) -> Void in
            if access_token == nil {
                // 获取验证码错误，或者网络异常等，这里需要直接处理返回
            ...
            let basicAuth = "Bearer " + access_token!
            let urlstr = "https://api.cnblogs.com/api/blogposts/\(blogId)/body"
            let url = NSURL(string: urlstr)
            let request = NSMutableURLRequest(URL: url!)
            request.HTTPMethod = "GET"
            request.addValue(basicAuth, forHTTPHeaderField: "Authorization")
